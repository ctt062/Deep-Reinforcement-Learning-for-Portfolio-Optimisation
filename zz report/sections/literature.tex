...
\begin{equation}
\text{Payoff}_{\text{put}} = \max(S_T, K_{\text{put}}) - P_0
\end{equation}
...
\begin{equation}
\text{Payoff}_{\text{call}} = \min(S_T, K_{\text{call}}) + C_0
\end{equation}

where $C_0$ is the premium received. When combined, protective puts and covered calls form a \textit{collar strategy}, which bounds the portfolio's payoff within a defined range while potentially achieving near-zero net premium cost.
