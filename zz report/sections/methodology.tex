...
\begin{equation}
d_s = n \times L \times 2 + n + n_{\text{features}} = 18 \times 60 \times 2 + 18 + 36 = 2214
\end{equation}
...
The agent can allocate a hedge ratio $h_t^{\text{put}}$ to protective puts on the portfolio. The put payoff at expiration is:

\begin{equation}
\Pi_t^{\text{put}} = h_t^{\text{put}} \cdot V_t \cdot \max\left(0, \frac{K_{\text{put}} - S_T}{S_0}\right) - P_0
\end{equation}
...
\subsubsection{Covered Call Strategy}
...
\begin{equation}
\Pi_t^{\text{call}} = C_0 - h_t^{\text{call}} \cdot V_t \cdot \max\left(0, \frac{S_T - K_{\text{call}}}{S_0}\right)
\end{equation}

where $K_{\text{call}} = 1.04 \cdot S_0$ (4\% out-of-the-money) and $C_0$ is the premium received. The covered call strategy:
\begin{itemize}
    \item Generates income through option premiums during low-volatility periods
    \item Caps upside potential when portfolio value exceeds the strike price
    \item Provides a buffer against small losses through premium income
    \item Works synergistically with protective puts in a ``collar'' configuration
\end{itemize}

\subsubsection{Combined Options Overlay}

The total options P\&L combines both strategies:
\begin{equation}
\Pi_t^{\text{options}} = \Pi_t^{\text{put}} + \Pi_t^{\text{call}}
\end{equation}

The agent learns to dynamically adjust hedge ratios $h_t^{\text{put}}$ and $h_t^{\text{call}}$ based on market conditions, with maximum ratios capped at 25\% of portfolio value for each strategy.
