...
\begin{equation}
\text{Vol Scalar} = \min\left(1.0, \frac{\sigma_{\text{target}}}{\sigma_{\text{realized}}}\right)
\end{equation}
...
\begin{cases}
1.0 & \text{if } DD_t < 3\% \\
1.0 - \frac{DD_t - 0.03}{0.09 - 0.03} \cdot 0.9 & \text{if } 3\% \leq DD_t < 9\% \\
0.10 & \text{if } DD_t \geq 9\%
\end{cases}
\end{equation}

The final exposure multiplier combines both mechanisms:
\begin{equation}
\text{Exposure} = \text{Vol Scalar} \times \text{DD Scalar}
\end{equation}

This mechanism provides systematic risk control by:
\begin{enumerate}
    \item Scaling exposure inversely to realized volatility (volatility targeting)
    \item Starting position reduction at 3\% drawdown
    \item Reaching minimum 10\% exposure at 9\% drawdown
    \item Preserving capital while maintaining recovery participation
\end{enumerate}
