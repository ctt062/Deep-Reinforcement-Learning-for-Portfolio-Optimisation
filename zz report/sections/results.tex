...
Figure \ref{fig:training_curves} shows the training curves for both agents over 100,000 timesteps. DDPG's off-policy learning enables faster convergence and higher final rewards, while PPO's on-policy approach shows steadier but slower improvement.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/training_curves.png}
\caption{Training curves showing episode reward progression. DDPG (left) achieves faster convergence due to off-policy learning, while PPO (right) shows more gradual improvement with higher variance.}
\label{fig:training_curves}
\end{figure}

\subsection{Portfolio Weight Allocation}

Figure \ref{fig:weight_allocation} analyzes the portfolio allocation strategies learned by each agent.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/weight_allocation.png}
\caption{Portfolio weight allocation analysis: (a) Average sector allocation comparison, (b) DDPG sector allocation over time with COVID-19 adjustment, (c) PPO sector allocation over time with COVID-19 adjustment, (d) DDPG top 5 asset holdings, (e) PPO top 5 asset holdings, (f) Weight distribution comparison showing DDPG's concentration vs PPO's diversification.}
\label{fig:weight_allocation}
\end{figure}

Key observations from the weight allocation analysis:
\begin{itemize}
    \item \textbf{DDPG}: Technology (28\%), Healthcare (17.6\%), Index (13.2\%), Bonds (10.2\%)
    \item \textbf{PPO}: Technology (25.1\%), Index (14.9\%), Healthcare (14.9\%), Financials (10.7\%)
    \item \textbf{DDPG Top 5}: MSFT (7.4\%), AAPL (7.4\%), PFE (7.3\%), AGG (7.3\%), JNJ (7.3\%)
    \item \textbf{PPO Top 5}: V (5.8\%), AAPL (5.2\%), NVDA (5.2\%), UNH (5.1\%), SPY (5.1\%)
    \item DDPG shows higher healthcare allocation for defensive positioning
    \item PPO maintains more uniform allocations closer to equal-weight (5.56\%)
\end{itemize}
