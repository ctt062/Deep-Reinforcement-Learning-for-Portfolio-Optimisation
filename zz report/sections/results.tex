...
\caption{Portfolio weight allocation analysis: (a) Average sector allocation comparison, (b) DDPG sector allocation over time showing COVID-19 adjustment, (c) Top 5 asset holdings for DDPG, (d) Weight distribution showing DDPG's more concentrated vs PPO's more diversified approach.}
\label{fig:weight_allocation}
\end{figure}

Key observations from the weight allocation analysis:
\begin{itemize}
    \item DDPG allocates more heavily to Technology (33\%) and Bonds (15\%) for growth and hedging
    \item PPO maintains more balanced allocations across sectors, closer to equal-weight
    \item During the COVID-19 crash, both agents increased bond allocations for risk reduction
    \item DDPG's top holdings include TLT (8.9\%), NVDA (8.7\%), and SPY (7.3\%)
\end{itemize}
