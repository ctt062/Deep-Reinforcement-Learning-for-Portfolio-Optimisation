...
Figure \ref{fig:correlation_matrix} shows the correlation structure of asset returns during the training period. Notable patterns include:
\begin{itemize}
    \item High correlation among technology stocks (0.77-0.89)
    \item Negative correlation between bonds (TLT, AGG) and equities (-0.02 to -0.15)
    \item Gold (GLD) provides diversification with low correlation to most assets
\end{itemize}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/correlation_matrix.png}
\caption{Asset return correlation matrix during training period (2010-2018). Technology stocks show high positive correlation while bonds (TLT, AGG) provide diversification benefits with negative correlation to equities.}
...
\subsubsection{Unified Hyperparameters}

To ensure fair comparison between DDPG and PPO, we use unified hyperparameters where applicable. Table \ref{tab:unified_params} shows the shared configuration.

\begin{table}[htbp]
\centering
\caption{Unified hyperparameters for fair comparison}
\label{tab:unified_params}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Parameter} & \textbf{Value (Both Agents)} \\
\midrule
...
\subsubsection{DDPG-Specific Parameters}

\begin{table}[htbp]
\centering
\caption{DDPG-specific hyperparameters}
...
\subsubsection{PPO-Specific Parameters}
...
\textbf{Parameter} & \textbf{Value} \\
\midrule
Steps per update (n\_steps) & 2,048 \\
Number of epochs & 10 \\
