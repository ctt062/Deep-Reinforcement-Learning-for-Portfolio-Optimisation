"""
Quick comparison of trained models on extended test period.
"""
...
def main():
    print("\n" + "="*80)
    print("QUICK COMPARISON - EXTENDED TEST PERIOD (2019-2024)")
    print("="*80)
    
    # Load configuration with extended test period
    config_path = "configs/config_extended_test.yaml"
    ...
    # Train-test split with 50/50 split for extended test period
    ...
    # Models to evaluate - only existing trained models
    models_to_eval = [
        ('PPO Baseline', 'models/ppo_20251116_121651_final.zip', False, False),
        ('DDPG Baseline', 'models/ddpg_20251116_174546_final.zip', False, False),
    ...
    print(f"\n{'='*80}")
    print("COMPARISON RESULTS - EXTENDED TEST PERIOD (2019-2024)")
    print("="*80)
    ...
    if all_results:
        ...
        col_order = ['Total Return', 'Annualized Return', 'Annualized Volatility', 
                     'Sharpe Ratio', 'Max Drawdown', 'Calmar Ratio', 'Average Turnover']
        df = df[col_order]
        
        # Format percentages
        pct_cols = ['Total Return', 'Annualized Return', 'Annualized Volatility', 'Max Drawdown', 'Average Turnover']
        for col in pct_cols:
        ...
        # Restore original scale for checking
        for model in df.index:
            sharpe = df.loc[model, 'Sharpe Ratio']
            dd = df.loc[model, 'Max Drawdown'] / 100  # Convert back to decimal
            sharpe_check = "✓" if sharpe > 1.0 else "✗"
            dd_check = "✓" if dd < 0.10 else "✗"
            print(f"{model:25s} Sharpe: {sharpe_check} ({sharpe:.4f})  DD: {dd_check} ({dd*100:.2f}%)")
